...
def normalize_for_tts(text: str) -> str:
    ...
    # U+2212 MINUS: unary (attached to what follows) reads 负, binary reads 减
    text = _re.sub(r"(?<=\s)−(?=\S)", "负", text)
    text = _re.sub(r"\s*−\s*", " 减 ", text)
    text = text.replace("‖", " 对 ")          # KL(p‖q)
    text = _re.sub(r"\s*[→←]\s*", "，", text)  # list/nav arrows → pause
    return text
