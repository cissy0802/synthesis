...
### Syn 6. 记忆:一份没人存下的录像,和一张 1995 年的架构图 〔A/B〕 ✅ 2026-09
- 页面:`memory.html` × zh/en · <https://hub.cissychen.com/synthesis/memory.html>
- 共通点:三边**都没有存下那份录像**,存下的是有损的痕迹加一条找回它的线索,读出来的必须当场重建。人=碎片 + 海马别上的书签;模型=摊在千亿权重里的知识,没有一处存着原文;向量库=文本片段 + 语义坐标。硬后果:重建就得填补,填补就一定可能填错——填对叫记性好,填错在人这边叫**错误记忆**(洛夫特斯商场走失,约四分之一被试「记起」假事件),在机器那边叫**幻觉**,是同一条路径的两个出口(检索质量那页把两边接上:拿不到对的文档就只能编,recall@5=0.71 意味着 29% 的问题正确文档根本没进视野)。另两条:② 同一个天平——模式分离/模式补全 ↔ 稳定性-可塑性,后者已做成显式旋钮(EWC 的 λ);方向相反(脑要主动忘,网络要别被动全忘),争的是同一个权衡。③ 信心与准确度分离——挑战者号研究近半细节被改写、信心纹丝不动;DNA 洗冤案约七成涉及目击者误认。
- 承重证据:**互补学习系统(1995)**。灾难性遗忘 1989 年被发现(顺序学任务,学后面抹掉前面;二十行代码可见:A 准确率 0.95 → 学完 B 掉到 0.55),而大脑天天做这件事几乎不犯。CLS 点破那层对应:大脑不是一套系统而是两套,快的易覆盖 + 慢的稳,靠夜里压缩重放把前者教给后者。后来的工程只是把这张架构图做成产品——**经验重放至今几乎是持续学习最强的基线**,2017 年生成重放的灵感明说就是海马睡眠重放。要认的账:生成器逐代退化,长序列后旧任务照样崩。判据满足:六年的工程僵局由隔壁学科的解剖结构解决,三十年后仍是最优解。
- 互联换来了什么:① 工程拿到一条默认做法(大模型持续训练按比例混合新旧数据,而非只喂新数据;个人微调同理——留一份有代表性的旧数据混进新一轮,比复杂正则更稳);② AI 产品可从目击证词研究**整套提货**——列队逐一呈现、双盲主持、当场记录初始信心、事后不反馈;这四条没有一条在提高准确度,全在防污染与防信心被拔高,翻译过去是:别写诱导式提问、画像写入必带证据消息编号(内容与来源分开存)、事件日志必带时间戳(当场记录胜过事后回忆);③ 学习拿到能代入数字的东西(遗忘曲线排期;自测一周后约一半 vs 重读约三成;识别性理解 vs 生成性理解),而且反向也成立——画像与事件日志不设淘汰策略就会自己撑爆,「会忘是本事」在这里是字面意义的工程约束。
- 边界判据(五问):① **写入有取舍吗?**(全都留下的是日志,不是记忆)② **读出是取回还是重建?**(永不出错、永不填补的那是数据库;会填补才必然会编)③ **读这个动作会不会改写被读的东西?**(人=再巩固;向量库检索一万次,库里一个字不变)④ **有没有离线再加工?**(把新经历织进旧知识、把细节磨成规律——最要命的一条,RAG 完全没有;训练本身反而满足它)⑤ **会不会主动遗忘?**(有淘汰策略,还是只能撑爆)。逐个量:上下文窗口五条基本全不过,它对应的是工作记忆而非长时记忆,且缺了那个决定什么能上桌的指挥官——「窗口拉长=记性更好」是这一片最贵的误解;RAG 过第二条,第四条最致命(不消化);权重里的知识第四条过得最好,缺的是事件;agent 记忆工程显式回答了五问中的四问。**诚实的借用**:间隔重复工具——不打比方,直接算曲线并照着排期,是真提货。**跑偏的一档**:「组织的记忆」「城市的记忆」——追问下去会发现它要的其实是当场记录(事后回忆会被已知结果重塑),那是日志问题,处方完全不同。
- 分档:能算数的 → 测量(遗忘曲线排期、容量按块计、recall@k、稳定性-可塑性旋钮);机制说得清的 → 模型(「重建必然填补、填补必然可能填错」给得出可检验的预言:接受错误记忆与幻觉同源,就知道「让模型更谨慎」方向不对,能改的是不污染输入、不夸大信心、来源与内容分开存);只剩语气的 → 修辞。
- 源页:10 篇 / 6 站 —— `neuroscience/long-term-memory-topic4`、`working-memory-topic3` · `psychology/memory-reconstruction-day44` · `ai-ml/context-engineering-day8`、`continual-learning-day52`、`rag-systems-day4` · `super-individual/memory-state-day7`、`retrieval-quality-day43` · `meta-knowledge/learning-science-day24` · `mental-models/information-knowledge-day23`
- 与原路线图的偏差:原条目列了 `book-recommendations`(7 页),清点后未找到成篇的记忆主题页(相关内容分散在教育/学习类书单里,不足以承一节),这条腿由 `mental-models/information-knowledge-day23` 的识别性/生成性理解替代,接得更紧;`ai-ml` 的主锚从「8 页」收窄到三页(context 工程、持续学习、RAG),因为三样被叫做记忆的东西正好各占一页。
- 未用上的备选源:`ai-ml/semantic-search-day22`、`knowledge-editing-day49`(语义检索细节与改参数里的知识,后者其实是「编辑记忆」的好素材,篇幅所限未展开);`super-individual/graphrag-day48`、`rag-eng-day10`(知识图谱路线与 RAG 工程细节);`neuroscience/ref-hippocampus`(海马参考页,内容已被 Topic 4 覆盖);`deep-reading/in-search-of-lost-time-read61`(普鲁斯特的非自主记忆,是很好的第七条腿,但会把重心从「机制共通」拽向文学)。
- 反模仿:Syn 1 是「五个房间 + 编号九节」,Syn 5 是「词的旅行路线 + 不编号九节」。本篇从**两个失败场景**切入(助手推荐烤肉店 / 家人各记一个版本),按「人 → 机器 → 工程 → 学习科学 → 共通点 → 收益 → 五问 → 三档」组织,九节不编号;两张 SVG 一横一表——横向那张画三个系统走的是同一条重建路径,表格那张把五问 × 四个系统逐格勾出来,这是本线第一次用表格型 SVG。

### Syn 7. 因果:一张永远缺一半的表 〔C〕 ✅ 2026-09
- 页面:`causality.html` × zh/en · <https://hub.cissychen.com/synthesis/causality.html>
- 共通点:五个地方在解**同一道填空题**,题面一字不差——鲁宾的潜在结果表里,每一行都有一格(这个人如果没被干预会怎样)**原则上观测不到**,不是「还没测」。因果推断的根本难题本质是缺失数据问题。各领域的全部技艺,是让别的东西顶上那一格,区别只在「谁来顶、凭什么它有资格顶」:随机分组的对照组(凭掷硬币)/ 一次准随机外力(越战征兵抽签、出生季度)/ 对照组的时间趋势(平行趋势)/ 第三变量取值相同的人(暗道全被测到)/ 一段论证出来的另类历史(福格尔没有铁路的美国、中立的奥斯曼)。
- 承重证据:**后门准则与 d-分离**。珀尔之前,「该控制哪些变量」是各学科各自的手艺,而大家共享一条**错误**直觉——控制得越多越保险。d-分离是纯组合判定(只看路径通不通、哪些节点被观测,不碰积分),结论却是关于分布的;加上 do 算子,「因果效应能否从纯观测数据识别」第一次成为可计算命题。判据满足在于它推翻的那条法则在**四个领域被独立犯过、每次换一副面孔而无人认出是同一件事**:二战给返航飞机加装甲、大学录取里成绩与特长的假负相关、医院里两种病看似负相关、ML 只拿被点击样本训练——全部塌成一行字「按共果分层」。
- 互联换来了什么:① 工程直接提走经济学家的准实验工具箱(做不了 A/B 就找近乎随机的断点:只在某区域生效的配置、只影响部分用户的发布、一次机房故障;反过来灰度与金丝雀本就是亲手造的随机化);② ML 拿到一个诊断,解释「预测极准的模型一干预就崩」(哮喘患者肺炎死亡率更低——预测层面正确,当因果用会死人;特征重要性高≠改它能改结果);③ 历史学拿到「反事实可以被算」这件事本身——福格尔把英雄叙事变成有数字、因而可被具体挑战的命题;④ 读健康新闻的人拿到两把尺子:证据金字塔定级,以及 ARR/NNT(他汀 2.0%→1.5%,相对 25% 听着漂亮,NNT=200)。
- 边界判据(六问):① **你的反事实是什么?**(一句话说不清「和什么比」→ 这是评价不是因果主张)② **那一格是谁顶上的?**(随机对照组 / 准随机外力 / 对照组趋势 / 一堆控制变量 / 一段论证——每下一级折扣当场打掉)③ **你押的假设叫什么名字?**(排他性、平行趋势、暗道全被测到、轨迹成立;说不出名字通常是没意识到在押注)④ **控制的变量在图上待在哪?**(共因要堵、共果碰不得、中介一控就抹掉真效应——三条指令互相矛盾,决定用哪条的是图不是数据)⑤ **数据是全体还是过了某道关卡的幸存者?**(每道筛子都是隐蔽的按共果分层)⑥ **就算因果成立,它有多大?**(相对还是绝对、NNT 多少;统计显著≠临床显著)。前五条问「是不是因果」,第六条问「值不值得为它改变行为」,后者被漏掉的次数一点不比前者少。
- **真正的断裂点是历史那一级**:前四档都预设一个总体(可分组、可抽样、可谈平均),历史没有——N=1 不是样本太小,是根本没有可随机化的对象,整条斜坡塌掉。但**史学反事实不是劣质的因果推断**,它有自己一套挺硬的标准:看这个反事实需要**改动多少东西**,改一个决策比改一个结构可信(苏伊士那段「透支的财政地基不会改变,清算只会推迟而非取消」正是在分层;大分流的答复「换时间换地点」拒绝了单因论)。它做的是把不可检验的命题写成可被具体反驳的形式。另两处停下:营养学无法做终身随访 RCT(伦理与依从性),证据等级天生矮一档,成熟做法是与「当前最佳可得证据」共处;机制的诱惑——CAST 试验压住室性早搏(替代终点)却让死亡率翻倍。
- 分档:能随机就随机(门槛比想象低,抛硬币决定今天用不用番茄钟也算;一次灰度就是一次实验);随机不了就把押注说出口(结论是条件命题「在平行趋势成立的前提下」,配敏感性分析——假设违背到什么程度结论才翻转);连假设都说不清的是叙事(不是贬义,史学反事实就是高质量叙事,错只在标签贴错)。
- 源页:7 篇 / 5 站 —— `meta-knowledge/causal-inference-day67`(主锚:因果阶梯、混杂、对撞、do-演算与自然实验)、`epidemiology-day36`(HRT 反转、健康使用者偏倚、混杂/中介/对撞三角色) · `ai-ml/causal-inference-day36`(潜在结果、IV、DiD、可信度阶梯、预测≠因果) · `mathematics/graphical-models-day41`(d-分离、解释消解、后门准则可计算) · `health-longevity/evidence-literacy-day61`(证据金字塔、RRR/ARR/NNT、p 值三陷阱、CAST) · `history/industrial-revolution-day16`(福格尔铁路反事实、大分流无单一原因)、`empire-collapse-day15`(四个反事实案例、可拨动的决策层 vs 拨不动的结构层)
- 与原路线图的偏差:原条目写「待补:meta-knowledge Day 67 发布后作为主锚」——Day 67 已于 2026-07 发布,直接用作主锚,无需再等。`history` 原标「23 页,史学因果」,清点后发现史学因果**没有单独成篇的方法论页**,而是每篇末尾固定的「反事实 + 历史学争论」栏目——这反而更好用:它是同一道填空题在史学里的标准化产物,取工业革命与帝国崩溃两篇即可。
- 未用上的备选源:`mathematics/probability-paradoxes-day27`(辛普森悖论,已被 ai-ml 页覆盖) · `meta-knowledge/zombie-statistics-day66`、`inside-the-paper-day73`(僵尸统计与论文解剖,是第六问「效应有多大」的好补充,篇幅所限未展开) · `ai-ml/fairness-bias-day51`(公平性里的因果路径,可作 Syn 未来条目) · `history/climate-day20`(气候决定论,是史学单因论最好的靶子,但会把重心拽离共通点)。
- 反模仿:Syn 1「五个房间 + 编号九节」,Syn 5「词的旅行路线 + 不编号九节」,Syn 6「两个失败场景 + 九节不编号 + 表格型 SVG」。本篇用**一格 + 五种填法**为主轴,**七节不编号**,从两次真实反转切入(HRT 反号 / 福格尔的铁路),SVG 两张:纵向五级斜坡(每级标「顶替它的是什么 / 押的假设 / 能否被数据检验」),以及三联小图(共因 ✓ / 共果 ✗ / 中介 ✗)——本线第一次用「同一对象三个位置」的并置型 SVG。
