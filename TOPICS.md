...
- 源页:14 篇 / 8 站 —— `physics/entropy-arrow-day7`、`heat-temperature-day6`、`statistical-mechanics-day8`、`information-physics-day9` · `mathematics/information-theory-day13` · `cs-papers-deepread/mathematical-theory-of-communication-paper36` · `ai-ml/loss-optimization-day10`、`probability-information-day33` · `meta-knowledge/bioenergetics-day23` · `complexity-science/dissipative-structures-topic15` · `deep-reading/order-of-time-read18`、`flow-read15` · `mental-models/information-theory-day58`、`physics-thinking-day36`
- 增补(2026-08,由 `hub-coverage-audit` 首次体检发现):补进 4 篇本文发表后才出现的源页,全部落在既有章节而非新开一节。§2 补 1/√N 涨落律(给「10²³ 让概率律变铁律」一个能算的度量,并点明 N 一小涨落就回来当主角)→ `physics/statistical-mechanics-day8`;§3 补有噪信道编码定理(速率低于容量 C 时错误率可任意趋零,且香农只证存在不给构造,故人类又花了近半个世纪才造出来)→ `cs-papers-deepread/...paper36`;§6 承重证据那节收获最大——点名贝内特 1982,并改掉一处含糊:**收费的不是测量而是遗忘**(测量原则上可逆免费),另补 kT ln2 在室温下约 2.9×10⁻²¹ 焦、且与材料无关 → `physics/information-physics-day9`(本节原先没有源页卡片,一并补上);§7 补兰道尔地板与 AI 能耗(实际耗能是地板的一万到十万倍,所以「AI 耗电是不可逾越的物理墙」可被直接反驳);§8 补耗散结构自己要认的账(普里高津的最小熵产生定理只在近平衡线性区成立,而耗散结构全在区外;最大熵产生原理至今只是假说;「生命是耗散结构」为真但信息量有限,飓风烛焰也是)→ `complexity-science/dissipative-structures-topic15`。中文 5936 → 6927 字。
...
### Syn 3. 合作:三本账,和一张掀翻的桌子 〔A/C〕 ✅ 2026-07 · 增补 2026-09
...
- 断裂点(第零问 + 五问判据):⓪ 这张收益矩阵是测出来的,还是编出来的?(其余五问都默认矩阵已存在;真实冲突里那四个数往往测不出来、双方排序还可能不同。操作化检验:能不能说出「把这个数改成另一个,结论会怎么变」——答不上来就是在用博弈论的词汇复述已有判断)① 用哪种货币算账?(收益回流到基因 / 未来的你 / 第三方声誉——三种不能混记)② 有没有「未来的影子」?(短 → 理论预言背叛,别指望道德补缺)③ 需不需要一个有边界的「自我」做受益人?(需要 → 在算账;以消解自我为前提 → 掀了桌子,别硬塞「其实是自利」)④ 翻译成「其实是划算的自利」,原命题会不会塌?(不塌 = 本就是自利账;塌了 = 你在用廉价收敛偷走它)⑤ 讲「是」还是「应该」?(从 `rB>C` 推「所以应偏袒亲属」缺一步价值前提 = 自然主义谬误)。
- 源页:17 篇 / 11 站 —— `mental-models/game-cooperation-day28`、`strategy-day04`、`evolutionary-psychology-day53` · `meta-knowledge/game-theory-day11`、`trust-cooperation-day58`、`evolutionary-biology-day4` · `deep-reading/selfish-gene-read2` · `civics-geopolitics/war-and-peace-rules-day13`、`great-power-rivalry-day10` · `buddhism/bodhisattva-day11` · `leadership/office-politics-day6` · `philosophy/individual-society-day15`、`ethics-virtue-day5` · `psychology/evolutionary-psychology-day32` · `sales/trust-currency-day2` · `evolutionary-biology/evolution-of-cooperation-day8` · `complexity-science/evolutionary-games-cooperation-topic29`
- 增补(2026-09,hub-coverage-audit 体检发现):两个源页、五处落点。`evolutionary-biology/evolution-of-cooperation-day8` → §2 补吸血蝙蝠的实证(Carter & Wilkinson 2013:两年 20 只逐只禁食、48 次喂食,「过去喂过我多少」比亲缘更能预测谁会喂——在开篇那个蝙蝠巢里把 `r` 和 `w` 分开测且分出高下),§3 补「以牙还牙一场单挑都没赢过」与清洁鱼的非人类间接互惠证据(含证据强度打折)。`complexity-science/evolutionary-games-cooperation-topic29` → §4 补奥斯特罗姆(八原则里的边界/自监督/分级制裁三条,把第三本账从「必须有利维坦」修正为「必须有可信的代价来源」),§5 把「群体过大/未来阴影缩短」两条定性崩溃条件换成可算的 `w ≥ (5−3)÷(5−1) = 0.5` 与 `b/c > k`,§7 加第零问并给判据本身打两处折(惩罚是二阶公共品;Herrmann 2008 的反社会惩罚)。SVG 判据框同步改为「第零问 + 五问」。
